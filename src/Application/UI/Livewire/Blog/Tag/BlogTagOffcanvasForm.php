...
namespace Koneko\VuexyWebsiteAdmin\Application\UI\Livewire\Blog\Tag;

use Illuminate\Support\Facades\DB;
use Illuminate\Validation\Rule;
use Koneko\VuexyAdmin\Support\Livewire\Components\Form\AbstractFormOffCanvasComponent;
use Koneko\VuexyContacts\Application\Services\ContactCatalogService;
use Koneko\VuexyStoreManager\Services\StoreCatalogService;

/**
 * Class BlogTagOffcanvasForm
 *
 ...
 *
 * @package Koneko\VuexyTag\Livewire\Tags
 */
class BlogTagOffcanvasForm extends AbstractFormOffCanvasComponent
{
    /**
     ...
     */
    protected $listeners = [
        'editTag' => 'loadFormModel',
        'confirmDeletionTag' => 'loadFormModelForDeletion',
    ];
        ...
        switch ($mode) {
            ...
            case 'edit':
                return [
                    ...
                    'code'           => ['required', 'string', 'max:16', Rule::unique('Tags', 'code')->ignore($this->id)],
    ...
    protected function options(): array
    {
        ...
        return [
            'store_options' => $storeCatalogService->getCatalog('stores', '', ['limit' => -1]),
            'manager_options' => $contactCatalogService->getCatalog('users', '', ['limit' => -1]),
        ];
    }
